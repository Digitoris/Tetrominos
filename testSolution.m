...
for i = 1:numel(solution.id)
    ...
    if any([solution.y(i) solution.x(i)] + pieces(solution.id(i)).size > board.size)
        %fprintf('Solution failed at piece %i [id:%i], exceeding bounds\n',i,solution.id(i));
        return;
    end
    x = solution.x(i) + pieces(solution.id(i)).xData;
    y = solution.y(i) + pieces(solution.id(i)).yData;
